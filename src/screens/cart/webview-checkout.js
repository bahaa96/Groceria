...
class WebviewCheckout extends Component {
  ...
  handleResponse = request => {
    ...
    if (url.includes('/order-received/')) {
      navigation.replace(cartStack.webview_thank_you, {
        uri: `${url}&mobile=1&theme=${theme}&currency=${currency}`,
      });
    }

    if (url.includes('/order-pay/')) {
      navigation.replace(cartStack.webview_payment, {
        uri: `${url}&mobile=1&theme=${theme}&currency=${currency}`,
      });
    }
  ...
  render() {
    console.log('Checkout webview rendered');
    ...
    const uri = `${API}/checkout?${queryString.stringify(checkoutQuery, {
      arrayFormat: 'comma',
    })}`;
